...
strength high enough; there's a little meter that indicates the antenna's 
...
class Solution:
    def checkIncrease(self, input):
        with open(f"{input}") as f:
            depths = [int(itm.strip()) for itm in f.readlines()]
        incs = [1 if depths[i] > depths[i - 1] else 0 for i in range(1, len(depths))]
        return sum(incs)


    def checkIncrease2(self, input):
        with open(f"{input}") as f:
            depths = [int(itm.strip()) for itm in f.readlines()]
        wind_1 = sum(depths[:3])
        inc = 0
...
if __name__ == '__main__':
    sol = Solution().checkIncrease2('inputday1.txt')
    print(sol)