...
They take turns making sequences by reading aloud the 
...
each run of digits (like 111) with the number of 
...
talking about this sequence (that's Conway of Conway's Game of Life fame).
...
class Solution:
    def look_and_say(self, input: str, runs: int) -> int:
        placer = 0
        i = 0
        while i < runs:
            placer = 1
            ...
            while counter < len(input):
                if input[counter] == input[counter - 1]:
                    ...
                    if counter == len(input) - 1:
                        new_input += str(placer) + input[counter]
                else:
                    new_input += str(placer) + input[counter - 1]
                    if counter == len(input) - 1:
                        new_input += '1' + input[counter]
                    placer = 1
                counter += 1
...
if __name__ == '__main__':
    solution = Solution().look_and_say('1113222113', 40)
    print(solution)