...
The elves are running low on wrapping paper, and so 
they need to submit an order for more. They have 
a list of the dimensions (length l, width w, and height h) 
çof each present, and only want to order exactly as much as they need.
...
The elves are also running low on ribbon. Ribbon is all 
the same width, so they only have to worry about the length 
...
the cubic feet of volume of the present. Don't ask how 
...
class Solution:
    def order_wrapping_paper(self, input: str, part2=False) -> int:
        total_wrap_paper = 0
        total_ribbons = 0
        with open(f"{input}", 'rt') as file:
            for line in file:
                # line will look something like the following:
                # 4x23x21
                dims = line.split('x')
                length, width, height = int(dims[0]), int(dims[1]), int(dims[2])
                
                total_wrap_paper += (2 * length * width + 2 * length * height + 2 * height * width + 
                                    min(length * width, length * height, height * width))
                total_ribbons += length * width * height + 2 * min(length + width, length + height, height + width)
        if part2:
...
if __name__ == '__main__':
    solution = Solution().order_wrapping_paper("wrap_paper_dims.txt", part2=True)
    print(solution)
