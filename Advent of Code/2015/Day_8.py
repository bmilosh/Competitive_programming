...
and  plus two hexadecimal characters (which represents a single character 
...
in each original string literal. For example, for the strings above, 
the total encoded length (6 + 9 + 16 + 11 = 42) minus the characters 
in the original code representation (23, just like in the first part 
...
class Solution:
    def count_length(self, file: str, part2=False) -> int:
        code_rep = 0
        string_rep = 0
        ...
        with open(f"{file}", 'rt') as f:
            for line in f:
                ...
                # We count all the special characters in the string
                slash_count = line.count('\\')
                apos_count = line[:-1].count('\\"')
                x_spec_char_count = line[:-1].count('\\x')

                # Part 2 is handled here
                if part2:
                    memory_chars = len(line) 
                    code_rep += len(line) + 4 + apos_count + slash_count
                    string_rep += memory_chars
                    continue

                # We note the x-special characters that are not real (e.g., '\\\\x') and discount them
                not_real_x = [1 for i in range(2, len(line) - 1) if line[i:i+2] == '\\x' 
                                and line[i-1:i+1] == '\\\\' 
                                and (line[:i+1].count('\\') - line[:i+2].count('\\x') - 
                                    line[:i+1].count('\"') ) % 2 == 0]
                x_spec_char_count -= sum(not_real_x)

                # This gives us the length of the memory representation of the string:
                memory_chars = len(line[1:-1]) - (4 * x_spec_char_count) + x_spec_char_count 
                if apos_count == 0: 
                    memory_chars -= ((slash_count - x_spec_char_count) / 2) 
                else:
                    memory_chars -= apos_count + (slash_count - (x_spec_char_count + apos_count)) / 2 
                code_rep += len(line)
                string_rep += memory_chars

        return code_rep - string_rep


if __name__ == '__main__':
    solution2 = Solution().count_length('matchsticks_day8.txt', part2=True)
    print(solution2)