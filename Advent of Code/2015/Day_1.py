...
"snow" function is powered by stars, and he's fresh out! 
...
a little confusing. He starts on the ground floor (floor 0) 
...
class Solution:
    def traverse_floor(self, input: str) -> int:
        floor = 0
    ...
    def traverse_floor_part2(self, input: str) -> int:
        floor = 0
        with open(f"{input}", 'rt') as f:
            items = f.read()
            ind = 0
...
if __name__ == "__main__":
    solution = Solution().traverse_floor("input.txt")
    print(solution)

    # solution = Solution().traverse_floor_part2("input.txt")
    # print(solution)