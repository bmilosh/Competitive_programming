...
better model of determining whether a string is naughty or nice. 
...
For example:
    ...
    qjhvhtzxzqqjkmpb is nice because is has a pair that appears 
        twice (qj) and a letter that repeats with exactly one 
        letter between them (zxz).
    xxyxx is nice because it has a pair that appears twice and 
        a letter that repeats with one between, even though the 
...
class Solution:
    def find_nice_strings(self, input: str) -> int:
        checker = 0
        with open(f'{input}', 'rt') as f:
            for line in f:
                line = line.strip()
                if (any(line.__contains__(forbid) for forbid in ['ab', 'cd', 'pq', 'xy']) or
                        sum([1 for letter in line if letter in 'aeiou']) < 3):
                    continue
    ...
    def find_nice_strings2(self, input: str) -> int:
        checker = 0
        with open(f'{input}', 'rt') as f:
            for line in f:
                line = line.strip()
                ...
                for i in range(len(line) - 2):
                    if line[i] == line[i + 2]:
                        seen_triple = True
                    if line[i + 2:].__contains__(line[i:i + 2]):
                        seen_double = True
                    if seen_triple == seen_double == True:
                        checker += 1
                        break
        return checker
...
if __name__ == '__main__':
    solution = Solution().find_nice_strings2('nice_strings.txt')
    print(f"{solution} nice strings found.")